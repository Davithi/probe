...
window.addEventListener('load', () => {
    ...
    const popupContents = document.querySelectorAll('.popup-dialog__content-section');
    updData(devices)
    startTimer()
    ...
    document.addEventListener('click', e => {
        ...
        if (e.target.closest('.device-control_danger')) {
            ...
            popupContents[elPos].classList.add('popup-dialog__content-section_current');

            callPopup(popupWarn);
        }

        if (e.target.classList.contains('dialog-error__button')) {
            let currentContent = document.querySelector('.popup-dialog__content-section_current');
            let points = [...document.querySelectorAll('.popup-dialog__content-section')];
            ...
            if (elPos === 0 && e.target.hasAttribute('data-message')) {
                if (e.target.hasAttribute('data-correct')) {
                    ...
                    document.querySelector('.popup-dialog_good-message.popup-dialog_message-comp1').classList.add('_opened');
                    devices[elPos].classList.remove('device-control_danger');
                    devices[elPos].classList.add('device-control_cured');
                    devices[elPos].querySelector('.device-control__control-btn').textContent = 'Угрозы отсутствуют';
                    updData(devices);
                } else {
                    closePopup(popupWarn)
                    document.querySelector('.popup-dialog_bad-message.popup-dialog_message-comp1').classList.add('_opened');
                }
            } else if (elPos === 6 && e.target.hasAttribute('data-message')) {
                if (e.target.hasAttribute('data-correct')) {
                    closePopup(popupWarn)
                    document.querySelector('.popup-dialog_good-message.popup-dialog_message-comp7').classList.add('_opened');
                    devices[elPos].classList.remove('device-control_danger');
                    devices[elPos].classList.add('device-control_cured');
                    devices[elPos].querySelector('.device-control__control-btn').textContent = 'Угрозы отсутствуют';
                    updData(devices);
                    ...
                    closePopup(popupWarn)
                    document.querySelector('.popup-dialog_bad-message.popup-dialog_message-comp7').classList.add('_opened');
                }
            } else {
                if (e.target.hasAttribute('data-correct')) {
                    closePopup(popupWarn)
        ...
        if (!e.target.closest('.popup-dialog__card') && e.target.closest('.popup-dialog')) {
            let popup = e.target.closest('.popup-dialog') ? e.target.closest('.popup-dialog') : e.target.querySelector('.popup-dialog');

            if (popup.classList.contains('_opened')) {
                closePopup(popup)
            }
        } else if (!e.target.closest('.popup_mistake') && popupMistake.classList.contains('clickable')) {
            if (popupMistake.classList.contains('_opened')) {
                closePopup(popupMistake)
                window.setTimeout(() => {
                    popupMistake.classList.remove('clickable');
        ...
        } else if (e.target.classList.contains('popup-complete') && !e.target.classList.contains('popup-complete__banner') && popupComplete.classList.contains('_opened')) {
...
function updData(devices) {
    ...
    document.querySelector('.dangerous-devices').textContent = document.querySelectorAll('.device-control_danger').length;

    if (parseInt(document.querySelector('.dangerous-devices').textContent) === 0) {
        document.querySelector('.manage-devices__statistic .text').style.color = '#18875F';
        document.querySelector('.manage-devices__statistic .text').innerHTML = `угрозы отсутствуют [ 8 / 8 ]`;
...
function infectDevices() {
    const devices = document.querySelectorAll('.device-control');
    const infectedDevices = document.querySelectorAll('.device-control.device-control_danger, .device-control.device-control_cured');
    ...
    if (availableDevices.length >= 2) {
      ...
      randomIndexes.forEach(index => {
        ...
        availableDevices[index].querySelector('.device-control__control-btn').textContent = 'Есть проблемы';
      });
    }
...
function startTimer() {
    ...
    intervalId = setInterval(function() {
      ...
      if (time < 0) {
        document.querySelector('.desk-timer').classList.remove('desk-timer_low-time');
        ...
        if (time <= 5) {
            document.querySelector('.desk-timer').classList.add('desk-timer_low-time');
        } else {
            document.querySelector('.desk-timer').classList.remove('desk-timer_low-time');
        }
      }
