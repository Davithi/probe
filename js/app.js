...
window.addEventListener('load', () => {
    ...
    document.getElementById('accounts-non-checked').textContent = accountCards.length;
    ...
    document.addEventListener('click', e => {
        ...
        if (e.target.classList.contains('accounts-answears__button')) {
            ...
            let answears = [...document.querySelectorAll('.accounts-answears__button')]
            let choice = answears.indexOf(e.target);
            let correctAnswear = document.querySelector('.accounts-card_current').dataset.correct;

            if (choice === parseInt(correctAnswear)) {
                document.querySelector('.accounts-card_current').classList.remove('accounts-card_active');
                document.querySelector('.accounts-card_current').classList.remove('accounts-card_current');
                ...
                if (cards.length - 1 >= 0) {
                    cards[parseInt(cards.length - 1)].classList.add('accounts-card_current');
                }
                if (cards.length - 2 >= 0) {
                ...
                if (cards.length === 0) {
                    ...
                    document.querySelector('.accounts-cards').innerHTML = `<p class="workflow__paragraph">Все данные актуализированы.</p><p class="workflow__paragraph">Вы проверили все учётные записи сотрудников!</p>`
                }

                document.getElementById('accounts-non-checked').textContent = cards.length;
            } else {
                callPopup(popup);
            }
        }
    ...
    // for popup
    document.addEventListener('click', e => {
        if (!e.target.closest('.popup') && !e.target.closest('.button_low-priority') && popup.classList.contains('clickable') && popup.classList.contains('_opened')) {
    ...
    document.addEventListener('keydown', function(event) {
        if (popup.classList.contains('_opened') || popupFinished.classList.contains('_opened')) {
            if (event.key == 'Escape') {
                popup.classList.remove('_opened');
                popup.classList.remove('clickable');
            }
...
function sortCards() {
    ...
    for (let i = 0; i < accountCards.length; i++) {
        ...
        if (i + 1 === accountCards.length) {
            document.querySelector('.accounts-cards').style.transform = `translateX(-${koef / 2}px)`
        }
    }
}